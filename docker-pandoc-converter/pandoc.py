...
def convert_file():
    ...
    try:
        with open(filepath, "r", encoding="utf-8") as f:
            ...
            data = {"from": from_fmt, "to": to_fmt, "math": math_option}
        ...
        if response.status_code == 200:
            ...
            messagebox.showerror(
                "Conversion Failed", response.json().get("error", "Unknown error")
            )
...
ttk.Combobox(
    root, textvariable=operation_var, values=list(OPERATIONS.keys()), width=45
).grid(row=1, column=1, sticky="w")

tk.Label(root, text="Math Engine:").grid(row=2, column=0, sticky="e")
ttk.Combobox(root, textvariable=math_var, values=["--mathjax", "--mathml", ""]).grid(
    row=2, column=1, sticky="w"
)

tk.Button(root, text="Convert", command=convert_file, bg="#4CAF50", fg="white").grid(
