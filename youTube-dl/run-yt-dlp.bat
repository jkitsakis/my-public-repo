...
goto start
:nothere
echo You either dont have, renamed, or moved the yt-dlp, ffprobe or ffmpeg executable. Please leave it in the same folder as this batch file
pause
exit
:start
cls
...
echo This is nothing new I'm just trying to make it easier to use the yt-dlp executable
echo yt-dlp Version:
yt-dlp --version
echo 1) Download a video/playlist (video)
echo 2) Download a video/playlist (mp3)
echo 3) View the main yt-dlp help (bit useuless here)
echo 4) Update the yt-dlp executable
...
choice
if errorlevel 2 goto start
yt-dlp -i --extract-audio --audio-quality 0 --embed-metadata --embed-thumbnail --audio-format mp3 -o "%(title)s.%%(ext)s%" %URL%
echo   ____ ___  __  __ ____  _     _____ _____ _____
echo  / ___/ _ \^|  \/  ^|  _ \^| ^|   ^| ____^|_   _^| ____^|
echo ^| ^|  ^| ^| ^| ^| ^|\/^| ^| ^|_) ^| ^|   ^|  _^|   ^| ^| ^|  _^|
echo ^| ^|__^| ^|_^| ^| ^|  ^| ^|  __/^| ^|___^| ^|___  ^| ^| ^| ^|___
