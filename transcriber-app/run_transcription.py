...
def transcribe_with_whisper(wav_path, model_path, language_code):
    ...
    print(
        f"🔠 Running whisper with model: {os.path.basename(model_path)} and language: {language_code}"
    )
...
def diarize(wav_path):
    ...
    print(
        f"🔁 Diarizing in {total_samples // chunk_samples + 1} chunks of {window_duration}s..."
    )
    ...
    for start in range(0, total_samples, chunk_samples):
        ...
        try:
            ...
            print(
                f"🧠 RAM used: {psutil.virtual_memory().percent}% - Collected embedding"
            )
    ...
    if len(embeddings) < 2:
        print(
            "⚠️ Not enough valid embeddings for clustering. Assigning Speaker 0 to all."
        )
        ...
        for n_clusters in range(2, 5):
            try:
                kmeans = MiniBatchKMeans(
                    n_clusters=n_clusters, batch_size=64, random_state=42
                )
