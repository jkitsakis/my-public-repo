...
Creates only:
    ...
    ├── downloaded_songs/
    ├── merged_playlist.m3u
    ├── merged_playlist_tracks.csv
    ├── merged_playlist_tracks.json
    └── summary.json
...
def load_existing_metadata(output_dir: Path) -> Dict[str, Dict[str, Any]]:
    ...
    for path in [output_dir / "merged_playlist_tracks.json"]:
        if path.exists():
            try:
    ...
    csv_path = output_dir / "merged_playlist_tracks.csv"
    if csv_path.exists():
        try:
...
def write_library_outputs(input_dir: Path, output_dir: Path, merged_songs: Path, metadata_by_key: Dict[str, Dict[str, Any]], summary_extra: Dict[str, Any]) -> None:
    ...
    m3u_path = output_dir / "merged_playlist.m3u"
    ...
    json_path = output_dir / "merged_playlist_tracks.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Operation 1: Shazam unknown MP3s, update ID3 tags/lyrics, and rename files in-place."
    )
    parser.add_argument("folder", nargs="?", help="Folder containing unknown MP3 files. If omitted, a folder dialog opens.")
    parser.add_argument("--no-lyrics", action="store_true", help="Skip LRCLIB lyrics lookup.")
    parser.add_argument("--no-repair", action="store_true", help="Skip ffmpeg MP3 repair.")
    parser.add_argument("--no-rename", action="store_true", help="Keep current filenames.")
    parser.add_argument("--missing-only", action="store_true", help="Only write missing ID3 frames instead of overwriting existing ones.")
    parser.add_argument("--acoustid-key", default=os.getenv("ACOUSTID_API_KEY", ""), help="Optional AcoustID API key for fallback identification.")
    args = parser.parse_args()

    folder = Path(args.folder).expanduser().resolve() if args.folder else pick_folder_gui()
    if not folder:
    ...
    asyncio.run(
        tag_unknown_folder(
            ...
            overwrite=not args.missing_only,
        )
    )
